<script>
    ...
    $(document).on('click', '.view_msg', function() {
        ...
        var to = $(this).attr('data-to-name');
        var from = $(this).attr('data-from-name');
        var to_user = $(this).attr('data-to-user');
        var from_user = $(this).attr('data-from-user');
        var to_user_online = $(this).attr('data-to-user-online');
        var from_user_online = $(this).attr('data-from-user-online');
        $("#to_u_profile_url").attr("href",
            "https://afromedium.nl/search_results.php?display=profile&amp;uid=" + atob(to_user));
        $("#from_u_profile_url").attr("href",
            "https://afromedium.nl/search_results.php?display=profile&amp;uid=" + atob(from_user));
        //note
        $("#to_note_modal").attr("data-user-id", atob(to_user));
        $("#to_note_modal").attr("data-user-type", 'to');
        $("#from_note_modal").attr("data-user-id", atob(from_user));
        $("#from_note_modal").attr("data-user-type", 'from');

        getUser(to_user, 'to');
        get_note(atob(to_user), 'to');

        getUser(from_user, 'from');
        get_note(atob(from_user), 'from');
        ...
        if (to_user_online != '') {
            ...
            $('#to_user_online3').removeClass('d-none');
            $('#to_user_online2').removeClass('chart-online');
            $('#to_user_online2').addClass('chart-online');
        } else {
            //$('#to_user_online').removeClass('chat-user-online');
            $('#to_user_online3').removeClass('d-none');
            $('#to_user_online3').addClass('d-none');
        }

        $('#from_user').text(from);
        $('#to_user').text(to);
        getMessages(id, from_user, to_user, 200, 1);
                ...
                if (data.error == '') {
                    if (type == 'to') {
                        ...
                        if (data.data.profile != '' && typeof(data.data.profile) != "undefined") {
                            if (data.data.profile.photo_id != '' && typeof(data.data.profile.photo_id) !=
                                "undefined") {
                                to_profile_pic = "https://www.afromedium.nl/_files/photo/" + data.data
                    ...
                    if (type == 'from') {
                        ...
                        if (data.data.profile != '') {
                            if (data.data.profile.photo_id != '') {
                                from_profile_url = "https://www.afromedium.nl/_files/photo/" + data.data
                                    .user_id + "_" + data.data.profile.photo_id + "_b.jpg";
                    ...
                    if (total != data.data.total_count) {
                        ...
                        $.each(resultArray, function(key, value) {
                            ...
                            if (atob(to_user) == value.from_user) {
                                ...
                                //     moment(
                                //         value.born).format('DD-MM-YYYY,h:mm a') + '</p>' +
            ...
            if (From != '' && typeof(From) != "undefined" && From !== null && To != '' && typeof(To) != "undefined" &&
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.error == '') {
                            $('#text_message').val('');
                            getMessages(btoa(Id), btoa(To), btoa(From), 200);
                        } else {
                    ...
                    error: function(err) {
                        swal("Error", "Message not sent Successfully! ," + err, {
                            icon: "error",
                        });
                    }
                }); // ajax call closing
